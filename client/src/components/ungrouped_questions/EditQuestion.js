...
export default function EditQuestion (props) {
  ...
  React.useEffect(() => {
    ...
    if (questionType === 'ungroup') {
      ...
      const question = questionsInQuestionGroup.filter(
  ...
  if (questionType === 'group') {
    ...
    selectedQuestion = questionsInQuestionGroup.filter(
      question => question._id === props.questionID
    )
  ...
  async function updateQuestion (event) {
    ...
    if (questionType === 'ungroup') {
      ...
      // questions in question group update logic
      // get the index of selected question
      const indexOfSelectedQuestion = questionsInQuestionGroup.findIndex(
        question => question._id === props.questionID
      )
      // delete the selected question
      const prevQuestionWithoutSelected = questionsInQuestionGroup.filter(
        question => question._id !== props.questionID
      )
      ...
      // insert the updated question to array of questions in question group
      const updatedQuestions = insert(
        prevQuestionWithoutSelected,
      ...
      // call an update
      await fetch(
        `http://localhost:5000/grouped-question/update/${props.questionGroup._id}`,
        {
          method: 'POST',
