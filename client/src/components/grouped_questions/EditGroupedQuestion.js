...
export default function EditGroupedQuestion (props) {
  ...
  async function updateQuestion (event) {
    ...
    const isGroupNameExist = await Checker.hasGroupNameDuplicate(
      formData.groupName
    )
    let updateThis = null
    ...
    if (updateThis === true) {
      await fetch(
        `http://localhost:5000/grouped-question/update/${formData._id}`,
        {
          method: 'POST',
              ...
              {formData.referenceType === 'text' ? (
                <label className='flex flex-vertical gap-sm'>
                  ...
                  <textarea
                    ...
                    value={formData.questionReference}
                    onChange={changeFormData}
                    onBlur={formatFormData}
                    required
                  />
                ...
                <label className='flex flex-vertical gap-sm'>
                  Image as Reference
                  <input
                    name='imageUrlAsReference'
                    value={formData.imageUrlAsReference}
                    onChange={changeFormData}
                    onBlur={formatFormData}
