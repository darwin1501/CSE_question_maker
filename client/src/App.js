...
function App () {
  ...
  const [
    showCreateUngroupedQuestion,
    setShowCreateUngroupedQuestion
  ] = React.useState(false)
  const [
    showEditUngroupedQuestion,
    setShowEditUngroupedQuestion
  ] = React.useState(false)
  ...
  const [
    tableUngroupedQuestionCount,
    setTableUngroupedQuestionCount
  ] = React.useState(0)

  const [
    showCreateGroupedQuestion,
    setshowCreateGroupedQuestion
  ] = React.useState(false)
  ...
  const [
    showCreateQuestionInGroup,
    setShowCreateQuestionInGroup
  ] = React.useState(false)
  ...
  const [
    questionIDOfQuestionGroup,
    setQuestionIDOfQuestionGroup
  ] = React.useState('')
  const [
    openEditQuestionInQuestionGroup,
    setOpenEditQuestionInQuestionGroup
  ] = React.useState(false)
  ...
  React.useEffect(() => {
    async function getQuestions () {
      ...
      const numericalCount = questions.filter(
        questions => questions.questionType === 'Numerical'
      ).length
      const analyticalCount = questions.filter(
        questions => questions.questionType === 'Analytical'
      ).length
      const verbalCount = questions.filter(
        questions => questions.questionType === 'Verbal'
      ).length
      const philConCount = questions.filter(
        questions => questions.questionType === 'Philippine Constitution'
      ).length
      const ra6713Count = questions.filter(
        questions => questions.questionType === 'RA 6713'
      ).length
      const envProtectionCount = questions.filter(
        questions =>
          questions.questionType === 'Environment management and protection'
      ).length
      const humanRightsCount = questions.filter(
        questions =>
  ...
  React.useEffect(() => {
    async function getGroupedQuestions () {
      ...
      questionGroup.forEach(
        questionData =>
          (totalQuestionCount += Object.keys(questionData.questions).length)
      )

      questionGroup
        .filter(questions => questions.questionType === 'Numerical')
        .forEach(
          question => (numericalCount += Object.keys(question.questions).length)
        )

      questionGroup
        .filter(questions => questions.questionType === 'Analytical')
        .forEach(
          question =>
            (analyticalCount += Object.keys(question.questions).length)
        )

      questionGroup
        .filter(questions => questions.questionType === 'Verbal')
        .forEach(
          question => (verbalCount += Object.keys(question.questions).length)
        )
      ...
      questionGroup
        ...
        .forEach(
          question => (philConCount += Object.keys(question.questions).length)
        )

      questionGroup
        .filter(questions => questions.questionType === 'RA 6713')
        .forEach(
          question => (ra6713Count += Object.keys(question.questions).length)
        )
      ...
      questionGroup
        ...
        .forEach(
          question =>
            (envProtectionCount += Object.keys(question.questions).length)
        )
  ...
  function LoadTable () {
    if (changeQuestionToGroup === false) {
      return ungroupQuestionsCount.totalQuestions > 0 &&
        tableUngroupedQuestionCount > 0 ? (
        <QuestionTable
      ...
      return tableGroupQuestionCount > 0 ? (
  ...
  async function findUngroupedQuestion (event) {
    ...
    if (question !== '') {
      const response = await fetch(
        `http://localhost:5000/questions/search/${question}`
      )
  ...
  async function findGroupedQuestion (event) {
    ...
    if (question !== '') {
      const response = await fetch(
        `http://localhost:5000/grouped-questions/search/${question}`
      )
        ...
        <div className='main-container'>
          {
            <Dashboard
              totalQuestions={
                ungroupQuestionsCount.totalQuestions +
                groupQuestionsCount.totalQuestions
              }
              numerical={
                ungroupQuestionsCount.numerical + groupQuestionsCount.numerical
              }
              analytical={
              ...
              philCon={
                ungroupQuestionsCount.philCon + groupQuestionsCount.philCon
              }
              ra6713={ungroupQuestionsCount.ra6713 + groupQuestionsCount.ra6713}
              envProtection={
                ungroupQuestionsCount.envProtection +
                groupQuestionsCount.envProtection
              }
              humanRights={
                ungroupQuestionsCount.humanRights +
                groupQuestionsCount.humanRights
              }
            />
          }
          {changeQuestionToGroup ? (
            <button
              ...
              onClick={toggleCreateQuestionGroup}
            >
            ...
            <button
              className='btn-create btn'
              onClick={toogleCreateUngroupedQuestion}
            >
              Create Question
            </button>
          )}
          ...
          <div style={{ display: 'flex', gap: '100px' }}>
            ...
            >
              ...
              <option value='Leonalyn Mutia Tayone'>
                Leonalyn Mutia-Tayone
              </option>
            </select>
          </div>
          <div className='table-container'>{<LoadTable />}</div>
          ...
          {showCreateGroupedQuestion && (
            <CreateGroupedQuestion
              toggleClose={toggleCreateQuestionGroup}
              updateUI={updateUIonGroupedTable}
            />
          )}

          {showEditGroupedQuestion && (
            <EditGroupedQuestion
